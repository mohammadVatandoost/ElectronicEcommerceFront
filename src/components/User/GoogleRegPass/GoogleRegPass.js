...
class GoogleRegPass extends Component {
    ...
    sendData = (event) => {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            console.log('password');console.log(this.state.password);
            axios.post('http://localhost:80/ariaelec/public/api/user/register',{token: this.props.match.params.token,password: this.state.data.password})
            .then((response) => {
