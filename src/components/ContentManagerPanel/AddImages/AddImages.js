...
class AddImages extends Component {
    ...
    sendData = (event) => {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            axios.post('http://localhost:80/ariaelec/public/api/cm/image/add',{token: this.props.token, image: this.state.imageFile },config)
                .then((res) => {
    ...
    getImages = () => {
        let url = 'http://localhost:80/ariaelec/public/api/cm/image/get?token='+this.props.token;
        console.log(url);
        axios.get(url)
            .then((response) => {
    ...
    render() {
        ...
        return (
            <div className="container text-right">
                ...
                <form method="post" action="http://localhost:80/ariaelec/public/api/cm/image/add"  encType="multipart/form-data">
