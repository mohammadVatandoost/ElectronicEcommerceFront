...
class ContentDetail extends Component {
    ...
    componentDidMount() {
        axios.post('http://localhost:80/ariaelec/public/api/cm/content/get', { id: this.props.match.params.id})
            .then((res) => {
    ...
    render() {
        ...
        // console.log(this.state.DraftEditor);
        // const editor = stateToHTML(this.state.DraftEditor);
        return (
          <div className="flex-row space-around content-detail">
              <div className="flex-item-2 flex-column">
                  {/*<ContentSmallSize/>*/}
                  {/*<ContentSmallSize/>*/}
                  {/*<ContentSmallSize/>*/}
              </div>
              <article className="flex-item-9">
                  <CardWrapper>
                      {/*<h1>{data.title}</h1>*/}
                      {showTest2}
                      <br/>
